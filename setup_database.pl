...
$multicom_db_tools_dir = "/storage/tools/MULTICOM_db_tools/";							        
...
print OUT "make clean\n\n";
print OUT "make NO_AFFINITY=1 USE_OPENMP=1 TARGET=NEHALEM \n\n";
...
if($gcc_version[0] ==4 and $gcc_version[1]<6) #gcc 4.6
{
	if(! -e "$multicom_db_tools_dir/tools/boost_1_38_0/install.done")
	{
		...
		{
			`mv $multicom_db_tools_dir/tools/boost_1_55_0 $multicom_db_tools_dir/tools/boost_1_55_0_original`;
			`ln -s $multicom_db_tools_dir/tools/boost_1_38_0 $multicom_db_tools_dir/tools/boost_1_55_0`;
		}
	}else{
...
if(-e "$multicom_db_tools_dir/tools/DNCON2/dncon2-v1.0.sh")
{
	...
	if($gcc_version[0] ==4 and $gcc_version[1]<6) #gcc 4.6
	{
		...
		print TMPT "$multicom_db_tools_dir/tools/DNCON2/freecontact-1.0.21/bin/freecontact < $install_dir/examples/T1006.aln\n\n";
	}else{
		while(<TMPI>)
		{
			$line = $_;
	...
	close TMPO;
	close TMPT;
	`chmod 755 $multicom_db_tools_dir/tools/DNCON2/dncon2-v1.0.sh`;
	`chmod 755 $multicom_db_tools_dir/tools/DNCON2/test_freecontact.sh`;
