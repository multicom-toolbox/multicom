        ...
        if line.startswith(prefix):
            ...
            for line in open(option_default):
                ...
                if "SOFTWARE_PATH" in line:
                    line = re.sub("SOFTWARE_PATH",install_dir)
                f.write(line+"\n")
            f.close()
...
option_list = "%s/installation/MULTICOM_configure_files/multicom_databases_packages.list";
check_file(option_list,'databases');
print "#########  Checking database files, done\n\n\n";

print("#########  (4) Checking tool files")
option_list = "%s/installation/MULTICOM_configure_files/multicom_tools_packages.list";
