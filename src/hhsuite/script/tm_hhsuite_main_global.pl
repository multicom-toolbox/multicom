...
#use hhsuite3.2 to search the sequence database to build multiple sequence alignments
#use hhsuite3.2 to search the template database to identify templates
##########################################################################
...
-d $hhsuite3_dir || die "can't find hhsuite3 dir: $hhsuite3_dir.\n";
-d $atom_dir || die "can't find atom dir.\n";
-d $pdb_db_dir || die "can't find $pdb_db_dir.\n";
-d $psipred_dir || die "can't find psipred dir.\n"; 
-f "${hhsuitedb}_hhm.ffdata" || die "can't find hhsearch database: ${hhsuitedb}_hhm.ffdata.\n";
...
if (-f "$fasta_file.global")
{
        print "The global alignment file exists, so directly go to alignment refinement and model generation.\n";
        goto MULTICOM;
}

if (-f "$name.hmm")
{
	print "The hmm profile of the target exists. Go to modeling directly...\n";
	goto MODEL; 
}
...
#system("$hhblits_dir/bin/hhblits -i $fasta_file -d $nr_db -oa3m $name.a3m -n $nr_iteration_num"); 
system("$hhsuite3_dir/bin/hhblits -i $fasta_file -d $nr_db -oa3m $name.a3m -n $nr_iteration_num"); 
...
system("$hhblits_dir/scripts/addss_v2020.pl $multicom_tool_dir $multicom_database_dir $name.a3m $name.ss.a3m -a3m"); 

#make hmm
$ENV{HHLIB}="$hhsuite_dir/lib/hh";
system("$hhsuite3_dir/bin/hhmake -i $name.ss.a3m -o $name.hmm"); 

MODEL:
#search shhm against the database
print "Search $name.hmm against $hhsuitedb...\n";
system("$hhsuite3_dir/bin/hhsearch -i $name.hmm -d $hhsuitedb -realign -global -o $name.glo.hhr"); 
print "Search is done.\n";

print "generate ranking list...\n";
system("$meta_dir/script/rank_templates.pl $name.glo.hhr $work_dir/$name.grank");
	
#parse the blast output
print "parse hhsearch output...\n"; 

system("$meta_dir/script/parse_hhsearch.pl $name.glo.hhr $fasta_file.global");

#validate local alignments
system("$meta_common_dir/script/validate_local.pl $fasta_file.global $atom_dir $fasta_file.global");
...
system("$meta_common_dir/script/local_global_align.pl $align_option $fasta_file.global $fasta_file $fasta_file.g");

#generate structure from local alignments
system("$meta_common_dir/script/local2model.pl $option_file $fasta_file $fasta_file.g .");
