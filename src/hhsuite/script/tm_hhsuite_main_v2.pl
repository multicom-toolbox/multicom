...
print "Generate alignments from the uniref hhsuite database using hhblits...\n";
...
print("$hhsuite3_dir/bin/hhsearch -i $name.hmm -d $hhsuitedb -realign -mact 0\n"); 

system("$hhsuite3_dir/bin/hhsearch -i $name.hmm -d $hhsuitedb -realign -mact 0"); 

if (! -f "$name.hhr")
{
	sleep(5);
	print "$name.hhr is not generated by hhsuite search. Try one more time...\n";
	system("$hhsuite3_dir/bin/hhsearch -i $name.ss.a3m -d $hhsuitedb -realign -mact 0"); 
	if (-f "$name.ss.hhr")
	{
...
if (! -f "$name.hhr")
{
	sleep(5); 
	print "$name.hhr is not generated by hhsuite search. Try one more time...\n";
	system("$hhsuite3_dir/bin/hhsearch -i $name.a3m -d $hhsuitedb -realign -mact 0"); 
}

print "Search is done.\n";

print "generate ranking list...\n";
system("$meta_dir/script/rank_templates.pl $name.hhr $work_dir/$name.rank");
	
#parse the blast output
print "parse hhsearch output...\n"; 

system("$meta_dir/script/parse_hhsearch.pl $name.hhr $fasta_file.local");
