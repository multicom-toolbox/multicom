...
#	if ($hour == 1 && $days[$day] == 0) #read hour 1, and is not updated today yet
	{
		...
		$date = `date`;
		print "start to update other  databases other than the main db on $date";
...
#			print "finish updateing sparks database.\n";	
			...
			#start to update hhpred database
			print "start to update hhpred database\n";
			`echo start to update hhpred database >> update.log`; 
			`date >> update.log`; 
			system("/home/chengji/casp8/hhpred/update_db/update_hhpred_all_CORRECT_2014.pl > hhpred.log");
			`date >> update.log`;
			`echo finish updating hhpred database >> update.log`;
			print "finish updateing hhpred database.\n";	
