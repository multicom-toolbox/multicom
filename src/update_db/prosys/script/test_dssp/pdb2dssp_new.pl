...
$dssp_dir = shift @ARGV;  #/scratch/jh7x3/multicom_beta1.0/tools/DSSP_64bit/
...
{ 
   ...
   if ( -f $full_pdb_name && ($full_pdb_name =~ /.*Z$/ || $full_pdb_name =~ /.*gz$/) )
   {
       ...
       print "${dssp_dir}dssp $unzip_file $dssp_file\n";
       $status = system("${dssp_dir}dssp $unzip_file $dssp_file");
       if ($status == 0) #succeed
       {
          `gzip -f $dssp_file`;
       }
