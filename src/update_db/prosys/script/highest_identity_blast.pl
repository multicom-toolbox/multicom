...
## added by jie to avoid long path in blast 
if(length($target_file)> 60)
{
  @tmp = split(/\//,$target_file);
  $file_name = pop @tmp;
  chomp $file_name;
  `cp $target_file $file_name.tmp`;
  #format the target dataset
  system("$blast_dir/formatdb -i $file_name.tmp"); 
  #blast against target dataset
  system("$blast_dir/blastall -i $query_file -d $file_name.tmp -p blastp -F F > $query_file.blast");
  `rm $file_name.tmp`;
}else{
  #blast against target dataset
