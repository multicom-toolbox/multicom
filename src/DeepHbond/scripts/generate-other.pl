...
# Modified by Tianqi, add ccmpred bug alert
...
my $check_flag = check_nan("ccmpred/$id.ccmpred");
my $count = 1;
my $date = `date`;
if($check_flag){
    while($check_flag){
        print("Found NAN Run CCMpred for $count more times....");
        system_cmd(RRPREDSCRIPT." $db_tool_dir alignment/$id.aln ccmpred");
        $check_flag = check_nan("ccmpred/$id.ccmpred");
        $count = $count+1;
        if($count == 4){last;}
    }
    if($check_flag){
        ...
        # my $msg1 = MIME::Lite->new(
                # From    => "$from",
                # To      => "$to",
                # Subject => "Error of CCMpred: $id!!!!",
                # Cc     => 'wutianqi1016@gmail.com',
                # Type    => 'multipart/mixed'
        # ) or die ("cannot create MIME object!");

        # $msg1->attach(
            # Type     =>'TEXT',
            # Data     =>"[$date] ccmpred/$id.ccmpred failed!!!!"
        # );
