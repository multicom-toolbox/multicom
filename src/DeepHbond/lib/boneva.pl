...
foreach my $row1(keys %xyz){
  ...
  if ($atom_type eq "hhbonds"){
    next if not ($atm1 eq "N");
  }
  if ($atom_type eq "ca"){
...
print "\n";
foreach my $rr (sort @rr_list){
  printf "%-30s", "".basename($rr)." (coverage)";
  foreach my $top (sort {$top_count{$a} <=> $top_count{$b}} keys %top_count){
    my %this_rr = %{$all_rr{"".basename($rr)." ".$top}};
    printf "%-10s", calc_coverage(\%this_rr);
  }
  print "\n";
}
...
print "\n";
foreach my $rr (sort @rr_list){
  printf "%-30s", "".basename($rr)." (mean-error)";
  foreach my $top (sort {$top_count{$a} <=> $top_count{$b}} keys %top_count){
    my %this_rr = %{$all_rr{"".basename($rr)." ".$top}};
    printf "%-10s", calc_fp_error(\%this_rr);
  }
  print "\n";
}
...
# Xd
print "\n";
printf "%-30s", "DIST. DISTRIBU. SCORE (Xd)";
foreach my $top (sort {$top_count{$a} <=> $top_count{$b}} keys %top_count){
  $top = "Top-$top" if $top ne "ALL";
  printf "%-10s", $top;
}
print "\n";
foreach my $rr (sort @rr_list){
  printf "%-30s", "".basename($rr)." (xd)";
  foreach my $top (sort {$top_count{$a} <=> $top_count{$b}} keys %top_count){
    my %this_rr = %{$all_rr{"".basename($rr)." ".$top}};
    printf "%-10s", calc_xd(\%this_rr);
  }
  print "\n";
}
...
# Spread
print "\n";
printf "%-30s", "Spread in contact-map";
foreach my $top (sort {$top_count{$a} <=> $top_count{$b}} keys %top_count){
  $top = "Top-$top" if $top ne "ALL";
  printf "%-10s", $top;
}
print "\n";
foreach my $rr (sort @rr_list){
  printf "%-30s", "".basename($rr)." (spread)";
  foreach my $top (sort {$top_count{$a} <=> $top_count{$b}} keys %top_count){
    my %this_rr = %{$all_rr{"".basename($rr)." ".$top}};
    printf "%-10s", calc_spread(\%this_rr);
  }
  print "\n";
}
...
# MCC
print "\n";
printf "%-30s", "MATTHEW'S CORRELATION COEFFICIENT (MCC)";
print "\n";
foreach my $rr (sort @rr_list){
  printf "%-30s", "".basename($rr)." (mcc)";
  my %this_rr = %{$all_rr_above_thres_conf{"".basename($rr)}};
  printf "%-10s", calc_mcc(\%this_rr);
  print "\n";
}

exit 0 if !$jaccard_selct;

print "\n";
print "JACCARD SIMILARITY MATRIX (Top-$jaccard_selct contacts)\n";
printf "%-20s", "";
foreach my $rr (sort @rr_list){
  printf "%-20s", "".basename($rr);
}
print "\n";
foreach my $rr1 (sort @rr_list){
  printf "%-20s", "".basename($rr1)." (N=0)";
  foreach my $rr2 (sort @rr_list){
    printf "%-20s", "".calc_jaccard_similarity($all_rr{"".basename($rr1)." ".$jaccard_selct}, $all_rr{"".basename($rr2)." ".$jaccard_selct}, 0);
  }
  print "\n";
}
print "\n";
...
print "\n";
foreach my $rr1 (sort @rr_list){
  printf "%-20s", "".basename($rr1)." (N=1)";
  foreach my $rr2 (sort @rr_list){
    printf "%-20s", "".calc_jaccard_similarity($all_rr{"".basename($rr1)." ".$jaccard_selct}, $all_rr{"".basename($rr2)." ".$jaccard_selct}, 1);
  }
  print "\n";
}
print "\n";
...
print "\n";
foreach my $rr1 (sort @rr_list){
  printf "%-20s", "".basename($rr1)." (N=2)";
  foreach my $rr2 (sort @rr_list){
    printf "%-20s", "".calc_jaccard_similarity($all_rr{"".basename($rr1)." ".$jaccard_selct}, $all_rr{"".basename($rr2)." ".$jaccard_selct}, 2);
  }
  print "\n";
}
print "\n";
foreach my $rr1 (sort @rr_list){
  printf "%-20s", "".basename($rr1)." (N=3)";
  foreach my $rr2 (sort @rr_list){
    printf "%-20s", "".calc_jaccard_similarity($all_rr{"".basename($rr1)." ".$jaccard_selct}, $all_rr{"".basename($rr2)." ".$jaccard_selct}, 3);
  }
  print "\n";
}
print "\n";
foreach my $rr1 (sort @rr_list){
  printf "%-20s", "".basename($rr1)." (N=4)";
  foreach my $rr2 (sort @rr_list){
    printf "%-20s", "".calc_jaccard_similarity($all_rr{"".basename($rr1)." ".$jaccard_selct}, $all_rr{"".basename($rr2)." ".$jaccard_selct}, 4);
  }
  print "\n";
}
...
atom  : string   : cb        : Atom type (ca/cb/all/heavyatoms/hhbonds)
