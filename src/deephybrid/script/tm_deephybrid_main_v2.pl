...
{
	`cp $hmm_model_file $hmm_db`;
}
#build the customized database of hmms for search
print("Generate the customized hmm database: $hhblits3_dir/bin/ffindex_build -as ${hmm_db}_hhm.ffdata ${hmm_db}_hhm.ffindex $hmm_db ...\n");
system("$hhblits3_dir/bin/ffindex_build -as ${hmm_db}_hhm.ffdata ${hmm_db}_hhm.ffindex $hmm_db");
...
if ($found_num > 0)
{
	...
	if (! -f "$name.ss.hmm")
	{
		system("$hhblits3_dir/bin/hhmake -i $name.ss.a3m -o $name.ss.hmm");
	}

	system("$hhblits3_dir/bin/hhsearch -e 1 -i $name.ss.hmm -d $hmm_db -realign -mact 0");
