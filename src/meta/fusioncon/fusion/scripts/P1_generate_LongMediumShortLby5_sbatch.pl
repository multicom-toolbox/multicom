...
foreach $file (@files)
{
  ...
  if(!(-e $fastafile) or !(-e $rrfile))
  {
	...
	print SH "cd $outfolder\n\n";
	print SH "sh /home/casp13/fusion/scripts/run_CASP13_fusion_withcontact_LongMediumShortL5.sh   $targetid  $fastafile   $work_dir $rrfile   &> dncon2_fusion_scratch_$targetid.log &\n\n";
	close SH;
