...
-f $novel_program || die "can't find $novel_program.\n";
...
system("$novel_program $name $fasta_file $work_dir"); 

for ($i = 1; $i <= $final_model_number; $i++)
{
	$model_file = "$work_dir/TOP5/Novel$i.pdb";
	if (-f "$model_file")
	{
...
print "Novel prediction is done.\n";
