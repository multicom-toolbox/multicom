...
while ($model_idx <= $easy_model_num && @easy_cm >= 5)
{
	...
	{
		system("$script_dir/cm_align_blast.pl $fasta_file $fasta_file.easy $cm_align_evalue $cm_max_gap_size $cm_min_cover_size hs$model_idx.pir");
	}
	else #use a new version of combination
	{
		...
		system("$meta_dir/script/hhsearch_align_comb.pl $script_dir $fasta_file $fasta_file.easy $cm_min_cover_size $cm_max_gap_size $cm_max_linker_size $cm_evalue_comb $adv_comb_join_max_size $cm_evalue_diff hs$model_idx.pir");  
	}

	open(PIR, "hs$model_idx.pir") || die "can't generate pir file from local alignments.\n";
	@pir = <PIR>;
	close PIR; 
	if (@pir <= 4)
	{
	...
	#if ($add_stx_info_rm_identical eq "yes")
	{
		...
		system("$script_dir/pir_proc_resolution.pl hs$model_idx.pir $chain_stx_info $rm_identical_resolution $fasta_file.pir.stx");
		`mv $fasta_file.pir.stx hs$model_idx.pir`; 

	}

	print "Use Modeller to generate tertiary structures...\n"; 
	system("$script_dir/pir2ts_energy.pl $modeller_dir $atom_dir $work_dir hs$model_idx.pir $cm_model_num");
	`rm model.log`; 

	if ( -f "$name.pdb")
	{
		`mv $name.pdb hs$model_idx.pdb`;
