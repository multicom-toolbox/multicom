...
for ($i = 283; $i <= 386; $i++)
{
	...
	$score_file = "$eva_dir/${target}all.ave";

	`mkdir consen_$target`;

	system("./script/consensus_global_local.pl ./script/ $spicker $tdir $seq_dir/$target $score_file consen_$target"); 	
