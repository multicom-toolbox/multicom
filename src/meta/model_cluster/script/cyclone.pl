...
#re-order target models
@order_models = ();
foreach $name (@close_models)
{
	foreach $record (@target_model)
	{
		if ($record =~ /$name/)
		{
			push @order_models, $record;
			last;
		}
	}
}

#push @new_rank, @order_models;
...
open(ROUND, ">$out_file.round");
print ROUND join("", @order_models);
close ROUND;
