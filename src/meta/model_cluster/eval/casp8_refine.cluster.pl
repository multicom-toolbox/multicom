...
$cmfr_dir = "/home/chengji/casp_home/software/prosys/eval/multicom_fr/";
-d $cmfr_dir || die "can't find cmfr_dir.\n";
$rank_dir = "/home/chengji/software/prosys/eval/multicom_jury/";
-d $rank_dir || die "can't find rank_dir.\n";
$meta_dir = "/home/chengji/casp8/meta/eval/";
...
$id = substr($target_name, 2);

$cmfr_eva = "$cmfr_dir/$id/$target_name.fr.eva";

$rank_eva = "$rank_dir/$id/$target_name.fr.eva";

$meta_eva = "$meta_dir/meta_$id/meta.eva";

while ($time < $time_out)
{
	#check if cmfr eva file there		
	if (-f "$cmfr_dir/$id/$target_name.fr.eva")
	{
		print "find cmfr models.\n";
		$cmfr_yes = 1;
	}
	if (-f "$rank_dir/$id/$target_name.fr.eva")
	{
		print "find rank models.\n";
		$rank_yes = 1; 
	}
	if (-f "$meta_dir/meta_$id/meta.eva")
	{
...
if ($cmfr_yes == 1)
{
	...
	while (@eva && $count <= $max)
	{
		...
		if (-f "$cmfr_dir/$id/$prefix.pir")
		{
			`cp $cmfr_dir/$id/$prefix.pir $output_dir/cmfr_$prefix.pir`;
		}
		$count++;
	}
}
...
if ($rank_yes == 1)
{
	...
	while (@eva && $count <= $max)
	{
		...
		if (-f "$rank_dir/$id/$prefix.pir")
		{
			`cp $rank_dir/$id/$prefix.pir $output_dir/rank_$prefix.pir`;
		}
		$count++;
	}
}
...
if ($meta_yes == 1)
{
	...
	while (@eva && $count <= $max)
	{
		...
		$orig_file = "$meta_dir/meta_$id/$name";
		if (-f $orig_file)
		{
		...
		if (-f "$meta_dir/meta_$id/$prefix.pir")
		{
			`cp $meta_dir/meta_$id/$prefix.pir $output_dir/$prefix.pir`;
		}
		$count++;
	}
}
