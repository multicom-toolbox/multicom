...
system("$script_dir/cm_main_comb_join_csblast.pl $cm_option $query_file $output_dir");
print "Done.\n";
...
open(PIR, "$output_dir/cm.pir");
@pir = <PIR>;
close PIR;

if (@pir > 4)
{
	print "There are multiple CM templates. Generate one model for each template...\n";
	system("$script_dir/main_cm_easy.pl $cm_option $query_file $output_dir");
	print "Done.\n";
}

#############################################################################################
#cm.pdb is not as good as cm1.pdb which takes into account the resolution of templates
#so disable it if cm1.pdb exists
#############################################################################################
if (-f "$output_dir/cm1.pdb")
{
	`mv $output_dir/cm.pdb $output_dir/cm.pdb.bak`;	
	`mv $output_dir/cm.pir $output_dir/cm.pir.bak`;	
}
