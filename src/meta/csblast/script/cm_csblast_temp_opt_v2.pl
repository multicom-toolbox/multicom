...
if ($nr_db ne "none")
{
	#print("$csblast_dir/csblast_static -i $seq_file -o $out_file -j $cm_blast_iteration -h $cm_including_evalue -e $cm_blast_evalue -d $temp_db -D $csblast_dir/K4000.lib --blast-path $blast_path"); 
	system("$csblast_dir/csblast_static -i $seq_file -o $out_file -j $cm_blast_iteration -h $cm_including_evalue -e $cm_blast_evalue -d $temp_db -D $csblast_dir/K4000.lib --blast-path $blast_path"); 
		#$cm_including_evalue = $value; 
	#system("$csblast_dir/csblast_static -i $seq_file -o $out_file -e $cm_blast_evalue -d $temp_db -D $csblast_dir/K4000.lib --blast-path $blast_path"); 

	#remove a few sentences from $out_file
	open(BLAST, $out_file);
	@blast = <BLAST>;
	close BLAST;
	$first_line = shift @blast;	
	shift @blast;
	shift @blast;
	shift @blast;
	shift @blast;
