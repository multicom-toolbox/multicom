...
system("$itasser_dir/I-TASSERmod/runNNNd.pl -libdir $itasser_dir -seqname $name -datadir $work_dir -usrname chengji"); 
...
close INIT;
shift @init; 
`mv initres.LOMETS initres.NNNd`; 

open(RANK, ">$name.NNNd.rank") ||die "can't create $name.NNNd.rank.\n";
...
for ($i = 1; $i <= 10; $i++)
{
	$org_align = "align_NNNd_$i.txt";
	$new_pir = "nnnd$i.pir";
	$atom_file = "threading_NNNd_$i.pdb";
	if (! -f $org_align)
	{
		next;
	}
	...
	system("$script_dir/clash_check_broken.pl $fasta_file nnnd$i.pdb > clash$i.txt"); 
	...
	foreach $entry (@clash)
	{
		...
		{
			print "nnnd$i.pdb's chain is broken, removed!\n";
			`mv nnnd$i.pdb nnnd$i.pdb.broken`; 
...
system("$meta_dir/script/filter_rank.pl $pdb_db_dir/pdb_cm $name.NNNd.rank $name.NNNd.filter.rank"); 

print "One LOMETS (NNNd) comparative modeling is done.\n";
