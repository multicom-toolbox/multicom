...
-d $hhblits_dir || die "can't find hhsearch dir.\n";
...
#make hmm
system("$hhblits_dir/bin/hhmake -i $name.ss.a3m -o $name.hmm"); 

#search shhm against the database
system("$hhblits_dir/bin/hhsearch -i $name.hmm -d $hhsearchdb -realign -mact 0"); 
...
system("$hhblits_dir/bin/hhsearch -i $name.hmm -d $hhsearchdb -realign -global"); 
...
print OPTION "\noutput_prefix_name=gbli\n";
close OPTION; 

##############################################################################################
#generate structure from local alignments
#disable this part due to its redundancy.
system("$meta_dir/script/local2model_v2.pl $option_file.global $fasta_file $fasta_file .");
#######################################################################################
##############################################################################################
...
for ($i = 1; $i <= $cm_model_num; $i++)
{
	...
	system("$meta_dir/script/standarize_pdb_code.pl gbli$i.pir gbli$i.pir"); 	
}
