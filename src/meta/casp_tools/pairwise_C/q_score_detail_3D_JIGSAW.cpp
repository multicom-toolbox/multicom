...
int main(int argc, char* argv[]){
	...
	ofstream COMPLOG("./3D-JIGSAW.log"); 
	...
	for(int i = 0; i < Ca.size(); i++){    //Ca.size() returns the number of models
		...
		for(int j = 0; j < Ca.size() ; j++){
			...
			double gdt_comp;
				...
				//comparision purpose
				tm_comp = strtod(results["tm_score"].c_str(), NULL) * second_model_len / length_query_seq;
				gdt_comp = strtod(results["gdt_ts"].c_str(), NULL) * second_model_len / length_query_seq;
				max_comp = strtod(results["max_sub"].c_str(), NULL) * second_model_len / length_query_seq;
                        }
			else{
			...
			if(models[i] == "/data/casp8_eva/tarball/T0388/3D-JIGSAW_V3_TS5"){
				COMPLOG << "BETWEEN" << model_id_i << "AND" << model_id_j << "tm:" << tm_comp << " " << "gdt:" << gdt_comp << " " << "max:" << max_comp << " " << "q:" << q_comp << endl;
			}	
		}
		if(Ca.size() > 1){
			tm_score /= (Ca.size() - 1);
