/* ATTENTION: (1) When running this program on multi-targets, each targets must have
   it's own EMPTY folder to store the TM_score output, i.e. the temp_dir argument,
   i.e., You CANNOT let different targets share the same temp folder!
   (2) That temp folder must be EMPTY before the program starts running!
*/
...
int PrintRank( map<double, string>& mymap, char *file, string target_id){
...
*/
	...
	//Check whether the temp_dir is empty
	cout << "second\n";
/*	vector<string> temp_files;
	GetDir(temp_dir, temp_files);
	if(temp_files.size() > 2){
		cerr << "The temp folder for tm_score output '" << temp_dir << "' must be EMPTY! And you cannot share the temp folder with more than one targets. Be careful if it contains .swp files, which will be treated as not empty!\n";
		exit(1);
	} 
*/
	...
	vector<string> models;
        map<double, string> tm_map;
        map<double, string> gdt_ts_map;
        map<double, string> max_sub_map;
	map<double, string> q_score_map;
	...
	for(int i = 0; i < Ca.size(); i++){    //Ca.size() returns the number of models
		...
		for(int j = 0; j < Ca.size() ; j++){
			...
			int second_model_len;
				...
				for(int i = 0; i < results["align_1"].size(); i++){
					align_1 = results["align_1"];
				}
                                for(int i = 0; i < results["align_2"].size(); i++){
			...
			cout << strtod(results["tm_score"].c_str(), NULL) << "\n";
                        cout << strtod(results["gdt_ts"].c_str(), NULL) << "\n";
                        cout << strtod(results["max_sub"].c_str(), NULL) << "\n";

			//start Q score calculation
			//cout << "MAIN i:" << i << "size " << Ca[i].size() << "\n";
			q_score += Calculate_Q(Ca[i], Ca[j], align_1, align_2) * num_common / length_query_seq ;				
			//cout << "OVER" << tm_score << " " << gdt_ts << " " << max_sub << " " << q_score << "\n\n";
		...
		tm_map[tm_score] = model_id_i;
		gdt_ts_map[gdt_ts] = model_id_i;
		max_sub_map[max_sub] = model_id_i;
		q_score_map[q_score] = model_id_i;
