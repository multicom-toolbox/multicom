...
while ($count <= 5)
{
	...
	system("$meta_dir/script/stx_model_comb_global.pl /storage/htc/bdm/jh7x3/multicom/tools/tm_score/TMscore_32 $casp_model_dir $name.score $fasta_file $output_dir/$name.pir 4 0.8 0.70");
...
#	$gdt = 0.5;
	...
	while (@pir < 10)
	{
		...
		system("$meta_dir/script/stx_model_comb.pl /storage/htc/bdm/jh7x3/multicom/tools/tm_score/TMscore_32 $casp_model_dir $name.score $fasta_file $output_dir/$name.pir 3 $length $gdt");
	...
	system("/storage/htc/bdm/jh7x3/multicom/src/prosys/script/pir2ts_energy.pl /storage/htc/bdm/jh7x3/multicom/tools/modeller-9.16/ $casp_model_dir $output_dir $output_dir/$name.pir 8");
