...
for ($i = 0; $i < @new_rank && $i < 5; $i++)
{
	...
	if (-f $model_file && -f $pir_file)
	{
		...
		system("/home/chengji/software/scwrl4/Scwrl4 -i $model_file -o $model_name.scw >/dev/null");
	        system("/home/chengji/casp8/model_cluster/script/clash_check.pl $fasta_file $model_name.scw > con_clash$i.txt");
