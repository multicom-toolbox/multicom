...
#@servers = ("hhsearch", "compass", "raptorx", "csiblast", "sam", "hmmer", "hmmer3", "psiblast", "newblast", "blast", "hhsearch15", "prc", "construct", "hhpred", "hhblits", "hhblits3", "ffas", "muster", "hhsearch151", "hhsuite3", "msa", "confold", "unicon3d","rosetta2", "rosetta3", "rosetta4", "rosetta5", "rosetta6", "rosetta7"); 
@servers = ("hhsearch", "compass", "raptorx", "csiblast", "sam", "hmmer", "hmmer3", "psiblast", "newblast", "blast", "hhsearch15", "prc", "construct", "hhpred", "hhblits", "hhblits3", "ffas", "muster", "hhsearch151", "hhsuite3", "msa","rosetta2", "rosetta3", "rosetta4", "rosetta5", "rosetta6", "rosetta7");  # manually remove "confold", "unicon3d"
...
for ($i = 0; $i < @servers; $i++)
{
	...
	if ($server eq "rosetta2")
	{
		...
		print "mkdir $output_dir/rosetta_common\n"; 
		`mkdir $output_dir/rosetta_common`; 
		chdir "$output_dir/rosetta_common"; 
		print "cd $output_dir/rosetta_common\n"; 
		print("$meta_dir/script/make_rosetta_fragment.sh $query_file abini $output_dir/rosetta_common $local_model_num &> $output_dir/run_fragment.log\n\n");
		system("$meta_dir/script/make_rosetta_fragment.sh $query_file abini $output_dir/rosetta_common $local_model_num &> $output_dir/run_fragment.log");
	...
	elsif ($kidpid == 0)
	{
		...
		{
			print("$hhsuite3_dir/script/tm_hhsuite3_main.pl $hhsuite3_option $query_file $server\n\n"); 
			system("$hhsuite3_dir/script/tm_hhsuite3_main.pl $hhsuite3_option $query_file $server"); 
		}
		elsif ($server eq "confold")
		{
			print("$confold_dir/script/tm_confold2_main.sh $confold_option $query_file $server\n\n"); 
			system("$confold_dir/script/tm_confold2_main.sh $confold_option $query_file $server"); 
		}
		elsif ($server eq "unicon3d")
		{
			print("$unicon3d_dir/script/tm_unicon3d_main.pl $unicon3d_option $query_file $server\n\n"); 
	...
	#		system("$muster_dir/script/tm_NNNd_main.pl $muster_option $query_file $server");
		...
		elsif ($server eq "rosetta")
		{
			if (length($qseq) <= 250)
			{
				...
				print("$meta_dir/script/rosetta_seed.pl $rosetta_dir $query_file $local_model_num 22222 $server\n");
				system("$meta_dir/script/rosetta_seed.pl $rosetta_dir $query_file $local_model_num 22222 $server");
				print("$meta_dir/script/rosetta_seed.pl $rosetta_dir $query_file $sim_num 22222 $server\n");
		...
		#	{
				...
				#copy fragment files
				print "cp -r $output_dir/rosetta_common/abini $server_dir\n"; 
				`cp -r $output_dir/rosetta_common/abini $server_dir`; 
				print("$meta_dir/script/run_rosetta_no_fragment.sh $query_file abini $server_dir $local_model_num >/dev/null\n\n");
				...
				#self model rosetta models
				print("$meta_dir/script/self_dir.pl $meta_dir/script/self.pl $server_dir\n\n");
		...
		elsif ($server eq "construct")
		{
			...
			print("$construct_dir/script/construct_hard_v9.pl $construct_option $query_file $output_dir\n\n");
...
if ($i == $thread_num && $post_process == 0)
{
	...
	@servers = ("hhsearch", "compass", "csiblast", "sam", "hmmer", "hmmer3", "psiblast", "newblast", "blast", "hhsearch15", "prc", "rosetta2", "rosetta3", "rosetta4", "rosetta5", "rosetta6", "rosetta7", "construct", "hhpred", "hhblits", "hhblits3", "ffas", "muster", "hhsearch151", "hhsuite", "hhsuite3", "raptorx", "msa", "confold", "unicon3d"); 
	for ($i = 0; $i < @servers; $i++)
	{
...
#	system("$prosys_dir/script/energy_models_proc.pl $meta_dir/script/eva_option $query_file $model_dir");
	...
	chomp $name;
	close FASTA;
#	system("$meta_dir/script/evaluate_models_meta.pl $prosys_dir $model_dir $name $query_file $model_dir/meta.eva");
