...
$tm_score_program = "/storage/htc/bdm/tools/MULTICOM_CLUSTER/sunflower/chengji/software/tm_score/TMscore_32";
...
for ($i = 1; $i <= 5; $i++)
{
	...
	if (-f $model_file)
	{
		system("/storage/htc/bdm/tools/MULTICOM_CLUSTER/sunflower/chengji/software/scwrl4/Scwrl4 -i $model_file -o $consensus_dir/$model_name-$i.pdb.scw >/dev/null");
		#system("/home/chengji/software/scwrl4/Scwrl4 -i $model_file -o $consensus_dir/$model_name-$i.pdb.scw");
		
		system("/storage/htc/bdm/tools/MULTICOM_CLUSTER/sunflower/chengji/casp8/meta/script/clash_check.pl $model_dir/$target_name.FASTA $consensus_dir/$model_name-$i.pdb.scw > $consensus_dir/clash$i.txt");
