...
$final_model_num = 5; 
$pdb2casp2 = "/exports/store2/casp14/tools/meta/script/pdb2casp.pl";
...
for ($i = 0; $i < @domain_dirs; $i++)
{
	...
	elsif ($idx1 >= 0 && $idx2 >= 0)
	{
		...
		{
			$domain_name = substr($dom_dir, $idx1+1);   	
		}
	}
	else
	{
	...
	#get the domain file
	$domain_file = $dom_dir . "/full_length/hhsuite/$domain_name.fasta";       
...
@servers = ("mcomb", "top_full_models"); 

@source_dirs = ("mcomb","top_full_models"); 

@prefixes = ("casp", "ave"); 

@destination = ("$output_dir/mcomb/","$output_dir/top_full_models/"); 
...
for ($m = 0; $m < @servers; $m++)
{
	...
	for ($i = 0; $i < $final_model_num; $i++)
	{
		...
		for ($j = 0; $j < $domain_num; $j++)
		{
			...
			{
				$model_file = "$domain_dir/$prefix$idx-casp.pdb";
			}
		...
		if ( -f "$comb_dir/$server$idx.pdb")
		{
			...
			system("/exports/store2/casp14/tools/scwrl4/Scwrl4 -i $comb_dir/$server$idx.pdb -o $comb_dir/$server$idx.pdb.scw >/dev/null");
			system("/exports/store2/casp14/tools/prosys/script/clash_check.pl $fasta_file $comb_dir/$server$idx.pdb.scw > $comb_dir/$server-clash$idx.txt");
