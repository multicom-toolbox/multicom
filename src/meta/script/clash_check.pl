...
for ($i = 0; $i < $length; $i++)
{
	for ($j = $i + 1; $j < $length; $j++)
	{
		...
		if ($dist < 0.1)
		{
			print "overlap: $aa1 ($pos1) <-> $aa2 ($pos2), dist = $dist\n";
		}
		elsif ($dist < 1.9)
		{
			print "servere clash: $aa1 ($pos1) <-> $aa2 ($pos2), dist = $dist\n";
		}
		elsif ($dist < 3.5)
		#elsif ($dist <= 3.5)
		{
			print "clash: $aa1 ($pos1) <-> $aa2 ($pos2), dist = $dist\n";
		}
		elsif ($dist > 4.5 && abs($pos1-$pos2) == 1)
		{
			print "chain broken: $aa1 ($pos1) <-> $aa2 ($pos2), dist = $dist\n";
		}
	}
}
