...
while ($i <= $count && @eva)
{
	...
	system("/home/chengji/casp8/model_cluster/script/clash_check.pl $fasta_file $out_dir/$model_name.pdb.scw > $out_dir/a_clash$i.txt");
...
system("/home/chengji/casp8/meta/script/iqa.pl $out_dir/meta.eva $mdir $fasta_file $out_dir/$target_name.iqa"); 
...
while ($i <= $count && @eva)
{
	...
	#repack the side chains 
	system("/home/chengji/software/scwrl4/Scwrl4 -i $model_file -o $out_dir/$model_name.pdb.scw >/dev/null");
	system("/home/chengji/casp8/model_cluster/script/clash_check.pl $fasta_file $out_dir/$model_name.pdb.scw > $out_dir/b_clash$i.txt");
	#if ($method eq "cm" || $method eq "fr")
	if (-f $pir_file)
	{
...
#		$model_file = "$mdir/$model_file";
		...
		if (-f $pir_file && -f $model_file)
		{
			...
			#do a self modeling
			system("~/casp9/casp_tools/self.pl $model_file $model_file.self"); 

			system("/home/chengji/software/scwrl4/Scwrl4 -i $model_file.self.pdb -o $model_file.scw >/dev/null");
			system("/home/chengji/casp8/model_cluster/script/clash_check.pl $org_fasta_file $model_file.scw > $mdir/clash$i.txt");
...
while (@eva)
{
	...
	if ($model_file =~ /hs1\./ || $model_file =~ /hh1\./ || $model_file =~ /com1\./ || $model_file =~/multicom1\./ || $model_file =~ /sam1\./ || $model_file =~ /prc1\./ || $model_file =~ /csiblast1\./ || $model_file =~ /hmmer1\./ || $model_file =~ /sp3_spem-comb/ || $model_file =~ /ss1\./ || $model_file =~ /psiblast1\./ || $model_file =~ /csblast1\./ || $model_file =~ /spem1\./ || $model_file =~ /hp1\./ || $model_file =~ /blits1\./ || $model_file =~ /gbli1\./ || $model_file =~ /ff1\./ || $model_file =~ /ap1/ || $model_file =~ /hg1/ || $model_file =~ /muster1/)
	{
		push @layer1_model, $model_file;
		push @layer1_pir, $pir_file;
	}
...
##############################Here, temporarily disabled, Dec. 6, 2011 ################################
