...
if (@ARGV != 5)
{
	die "need 5 parameters: prosys_dir (/storage/hpc/scratch/jh7x3/multicom/src/prosys/), modeller_dir (/storage/hpc/scratch/jh7x3/multicom/tools/modeller9v7/),  fasta file, multicom output directory (~/casp_roll/T0663), and number of domains\n";
}
...
foreach $model (@models)
{
	for ($jj = 0; $jj < 5; $jj++)
	{
		...
		system("/storage/hpc/scratch/jh7x3/multicom/src/meta/model_cluster/script/clash_check.pl $fasta_file $model_file.scw > $comb_dir/clash-$model$jj.txt");
