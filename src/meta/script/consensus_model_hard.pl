...
for ($i = 1; $i <= 5; $i++)
{
	...
	if (-f $model_file)
	{
		...
		system("/home/test/jie_test/multicom/tools/scwrl4/Scwrl4 -i $model_file -o $consensus_dir/$model_name-$i.pdb.scw >/dev/null");
		...
		system("/home/test/jie_test/multicom/src/meta/model_cluster/script/clash_check.pl $model_dir/$target_name.FASTA $consensus_dir/$model_name-$i.pdb.scw > $consensus_dir/clash$i.txt");
