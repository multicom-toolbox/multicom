...
$tm_score_program = "/storage/hpc/scratch/jh7x3/multicom/tools/tm_score/TMscore_32";
...
for ($i = 1; $i <= 5; $i++)
{
	...
	if (-f $model_file)
	{
		...
		system("/storage/hpc/scratch/jh7x3/multicom/tools/scwrl4/Scwrl4 -i $model_file -o $consensus_dir/$model_name-$i.pdb.scw >/dev/null");
		...
		system("/storage/hpc/scratch/jh7x3/multicom/src/meta/model_cluster/script/clash_check.pl $model_dir/$target_name.FASTA $consensus_dir/$model_name-$i.pdb.scw > $consensus_dir/clash$i.txt");
