...
#########################################################################
#
#               MULTICOM Protein Structure Evaluation System
#                         Author: Jianlin Cheng
#                         Start date: 1/13/2010
#                         Expected End date: 3/31/2010
#
#########################################################################
#This is a portion of multicom structure prediction system
#Revised from multicom_v3.pl
#It does not generate models. It only assess models. 
#It assumes all the models have been generated in full_length/meta.
#########################################################################
...
if ($DEBUG == 0)
{
#	system("$multicom_dir/script/multicom_server_v3.pl $meta_option_full_length $query_file $full_length_dir"); 
}
#########################################################################################################
...
##################################temporarily disable it for testing##########################################
	if ($DEBUG == 0)
	{
		...
		`mkdir $full_length_dir`; 
#		system("$multicom_dir/script/multicom_server_hard_v2.pl $meta_option_hard_domain $query_file $full_length_dir_hard"); 
	}
##############################################################################################################
...
if (@domain_start > 1) #multiple domain case
{
	...
	for ($i = 0; $i < @domain_start; $i++)
	{
		...
		if ($level eq "easy")
		{
			...
			if ($DEBUG == 0)
			{
#				system("$multicom_dir/script/multicom_server_v3.pl $meta_option_easy_domain domain$i.fasta $domain_dir"); 
			}
			#######################################################################################
		}
		else
		{
			...
			if ($DEBUG == 0)
			{
#				system("$multicom_dir/script/multicom_server_hard_v2.pl $meta_option_hard_domain domain$i.fasta $domain_dir"); 
			}
			#######################################################################################
		}
...
if (0) #############domain combination is disabled for model assessment
{
...
#if ($domain_split_comb == 1)
if (0) #assessment for each domain is disabled. 
{
	my $domain_num = @pir_seq; 
	for ($i = 0; $i < $domain_num; $i++)
	{
...
system("$multicom_dir/script/refine_model_tails_v2.pl /home/casp13/MULTICOM_package/cheng_group/bin/run_rosetta_refine.sh $full_length_dir/meta/$model_prefix.pir $full_length_dir/meta/$model_prefix.pdb  $refine_dir $refine_num 2>&1 1>/dev/null");  
...
system("$multicom_dir/script/multi_level_model_selection.pl $full_length_dir/meta/ $output_dir/full_length.dash $output_dir/select");  
...
{
#	system("$multicom_dir/script/combine_domains.pl $prosys_dir $modeller_dir $query_file $output_dir $domain_num"); 	
}

#generate sov scores for models, may be useful for very hard targets
system("/home/casp13/MULTICOM_package/casp8/sov/run_sov.pl $query_name $full_length_dir/meta/ $full_length_dir/meta/$query_name.sov"); 

#generate ss and sa matching scores
system("/home/casp13/MULTICOM_package/casp8/sov/get_ss_sa_score.pl $query_file $full_length_dir/meta/ $full_length_dir/meta/model_check > $full_length_dir/meta/$query_name.ss.sa"); 
...
for ($i = 1; $i <= 5; $i++)
{
        ...
        if (-f "$model_file.org")
        {
                system("/home/casp13/MULTICOM_package/software/scwrl4/Scwrl4 -i $model_file.org -o $mdir/casp$i.scw >/dev/null");
                system("/home/casp13/MULTICOM_package/casp8/model_cluster/script/clash_check.pl $query_file $mdir/casp$i.scw > $mdir/clash$i.txt");
