...
if (@domain_start > 1) #multiple domain case
{
	...
	for ($i = 0; $i < @domain_start; $i++)
	{
		...
		else
		{
			...
			if ($DEBUG == 0)
			{
				print("$multicom_dir/script/multicom_server_hard_vd.pl $meta_option_hard_domain domain$i.fasta $domain_dir\n\n"); 
...
print("$multicom_dir/script/analyze_alignments_v3.pl    $full_length_dir/meta/   $full_length_dir/meta/$query_name.align\n\n");
...
print("$multicom_dir/script/gen_dashboard_v2.pl $full_length_dir/meta/ $query_name $output_dir/full_length.dash\n\n");  
...
if ($domain_split_comb == 1)
{
	...
	for ($i = 0; $i < $domain_num; $i++)
	{
		...
		print("$multicom_dir/script/analyze_alignments_v3.pl    $model_dir   $output_dir/domain$i/$query_name.align\n\n");
		system("$multicom_dir/script/analyze_alignments_v3.pl    $model_dir   $output_dir/domain$i/$query_name.align");
		print("$multicom_dir/script/gen_dashboard_v2.pl $model_dir domain$i $output_dir/domain$i.dash\n\n"); 
...
print("$multicom_dir/script/combine_models_gdt_tm.pl $multicom_dir  $full_length_dir/meta/ $output_dir/full_length.dash $query_file $output_dir/mcomb/\n\n");
...
print("$multicom_dir/script/top2comb.pl $output_dir/mcomb/ $output_dir/mcomb/consensus.eva 0.88\n\n"); 
...
{
	print("$multicom_dir/script/convert2casp_v6.pl $prosys_dir $multicom_dir $output_dir $query_name $query_file $full_length_dir\n\n"); 
	system("$multicom_dir/script/convert2casp_v6.pl $prosys_dir $multicom_dir $output_dir $query_name $query_file $full_length_dir"); 

}
else
{
	print("$multicom_dir/script/convert2casp_v5.pl $prosys_dir $multicom_dir $output_dir $query_name $query_file\n\n"); 
...
`mkdir $eva_dir`; 
print("$human_qa_program $query_name $query_file $full_length_dir/meta $eva_dir >$eva_dir/qa.log 2>&1\n\n");
...
for ($i = 0; $i < @domain_start; $i++)
{
	...
	print("$human_qa_program $domain_name $domain_file $domain_dir/meta $eva_dir >$eva_dir/qa.log 2>&1\n\n");
...
if ($domain_split_comb == 1)
{
	...
	print("$multicom_dir/script/combine_domains_v4.pl $prosys_dir $modeller_dir $query_file $output_dir $domain_num\n\n"); 	
