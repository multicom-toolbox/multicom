...
for ($m = 0; $m < @servers; $m++)
{
	...
	for ($i = 0; $i < $final_model_num; $i++)
	{
		...
		if ( -f "$comb_dir/$server$idx.pdb")
		{
			...
			system("/home/casp13/MULTICOM_package/software/scwrl4/Scwrl4 -i $comb_dir/$server$idx.pdb -o $comb_dir/$server$idx.pdb.scw >/dev/null");
			system("/home/casp13/MULTICOM_package/casp8/model_cluster/script/clash_check.pl $fasta_file $comb_dir/$server$idx.pdb.scw > $comb_dir/$server-clash$idx.txt");
