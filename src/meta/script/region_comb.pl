...
#############################domain combination########################################
$cm_model_num = 5; 
$modeller_dir = "/storage/htc/bdm/jh7x3/multicom/tools/modeller9v7/"; 
$prosys_dir = "/storage/htc/bdm/jh7x3/multicom/src/prosys/";
$final_model_num = 5; 
...
for ($m = 0; $m < @servers; $m++)
{
	...
	for ($i = 0; $i < $final_model_num; $i++)
	{
		...
		if ( -f "$comb_dir/$server$idx.pdb")
		{
			...
			system("/storage/htc/bdm/jh7x3/multicom/tools/scwrl4/Scwrl4 -i $comb_dir/$server$idx.pdb -o $comb_dir/$server$idx.pdb.scw >/dev/null");
			system("/storage/htc/bdm/jh7x3/multicom/src/meta/model_cluster/script/clash_check.pl $fasta_file $comb_dir/$server$idx.pdb.scw > $comb_dir/$server-clash$idx.txt");
