...
#############################domain combination########################################
$cm_model_num = 5; 
$modeller_dir = "/data/jh7x3/multicom_github/multicom/tools/modeller9v7/"; 
$prosys_dir = "/data/jh7x3/multicom_github/multicom/src/prosys/";
$final_model_num = 5; 
...
for ($m = 0; $m < @servers; $m++)
{
	...
	for ($i = 0; $i < $final_model_num; $i++)
	{
		...
		if ( -f "$comb_dir/$server$idx.pdb")
		{
			...
			system("/data/jh7x3/multicom_github/multicom/tools/scwrl4/Scwrl4 -i $comb_dir/$server$idx.pdb -o $comb_dir/$server$idx.pdb.scw >/dev/null");
			system("/data/jh7x3/multicom_github/multicom/src/meta/model_cluster/script/clash_check.pl $fasta_file $comb_dir/$server$idx.pdb.scw > $comb_dir/$server-clash$idx.txt");
