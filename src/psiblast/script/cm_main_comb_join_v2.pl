...
-d $multicom_dir || die "can't find multicom psiblast predictor dir.\n";
if ($nr_dir ne "none")
{
...
if (-d $nr_dir )
{
	...
	print "check if nr database exists...\n"; 
	(-f "$nr_dir/$nr_db_name.phr" || -f "$nr_dir/$nr_db_name.pal") || die "can't find the nr database: $nr_dir/$nr_db_name.pal.\n"; 
	print "the nr database exists.\n";
	...
	system("$multicom_dir/script/cm_psiblast_temp_opt.pl $option_file $fasta_file $fasta_file.blast"); 
}
else
{
