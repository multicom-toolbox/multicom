...
if (@ARGV != 2)
{
	die "need two parameters: sequence file, output dir.\n";
}
...
if ($pos >= 0)
{
	$query_filename = substr($query_filename, $pos + 1);
}
print "input fasta file: $query_filename\n";
...
$hhsearch_local_alignment = "$work_dir/$query_filename.local.org";
...
else
{
	...
	$hhsearch_pir_file = "$work_dir/hhsuite1.pir";
	...
	if (-f $hhsearch_pir_file)
	{
		...
		if ($min_left_miss_region >= $min_domain_length)
		{
			...
			print LOG "the number of residues missed in the left domain based on the hhsuite1.pir\n\n";
			print "the number of residues missed in the left domain based on the hhsuite1.pir\n\n";
		}
	}
}
...
{
		print LOG "The right domain of $right_domain_missing residues is not covered. Suggested right domain size for domain splitting is $first_right_len.\n";
