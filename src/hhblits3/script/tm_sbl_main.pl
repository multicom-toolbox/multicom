...
print("$hhblits3_dir/bin/hhblits -i $fasta_file -d $hhblits3_pdb_hmm_db\n");
#system("$hhblits3_dir/bin/hhblits -i $name.hmm -d $hhblits3_pdb_hmm_db");
system("$hhblits3_dir/bin/hhblits -i $fasta_file -d $hhblits3_pdb_hmm_db");
...
#filter ranked templates according to the PDB library (not used anymore? double check. may remove this step)
...
system("$meta_dir/script/filter_alignments.pl $pdb_list_dir/pdb.list $fasta_file.local $fasta_file.local.fil");

`cp $fasta_file.local $fasta_file.local.org`;
`mv $fasta_file.local.fil $fasta_file.local`; 

#remove templates from rank file
system("$meta_dir/script/filter_rank_file.pl $pdb_list_dir/pdb.list $work_dir/$name.rank $work_dir/$name.rank.fil");
