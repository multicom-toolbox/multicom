...
#Modified from combine_pir_align_adv_multi.pl
#Author: Jianlin Cheng
#Date: 11/1/2005
###############################################################
if (@ARGV != 6 && @ARGV != 7)
{
	die "need 6 or 7 parameters: script dir, input pir file1, input pir file 2, min cover size(20), max linker size(10), join max size(5, optional), output file\n";
}
$param_num = @ARGV;
...
while (@pir2)
{
	...
	system("$script_dir/combine_pir_align_adv_join.pl $script_dir $out_file $out_file.alg $min_cover_size $max_linker_size $join_max_size $out_file"); 
}
