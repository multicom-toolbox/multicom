...
if (@ARGV != 7)
{
	die "need 7 parameters: script_dir, alignment_list_file, min_cover_size(20), stop_gap_size(20), max_linker_size(10), number_alignments_to_generate(5), output_prefix.\n";
}
...
while  ($real <= $num_comb && $num <= @ali_list)
{
	...
	for ($i = $num; $i < @ali_list; $i++)
	{
		...
		#Here there is a serious issue: the second alignment for combine_pir_align_adv.pl
		#must only contains one alignment. But this is not true for palign local alignments.
		#Need to create another script to handle the multiple alignments in pir2.
		#system("$script_dir/combine_pir_align_adv.pl $script_dir $current_align $this_align $min_cover_size $max_linker_size $current_align >/dev/null");
		system("$script_dir/combine_pir_align_adv_multi.pl $script_dir $current_align $this_align $min_cover_size $max_linker_size $current_align >/dev/null");
	}
	$num++;
	$real++; 
}
