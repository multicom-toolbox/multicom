...
@suffix = ("rank.hhs", "rank.com", "rank.ssm", "rank.sam", "rank.prc", "rank");
@sel_temps = ();

foreach $suff (@suffix)
{
	$rfile = "$query_dir/$qname.$suff"; 
	open(RANK, $rfile) || die "can't read ranked templates file.\n";
	@rank = <RANK>;
	shift @rank;
	close RANK;
	...
	for ($i = 0; $i < @rank; $i++)
	{
		...
		foreach $stemp (@sel_temps)
		{
			@entries = split(/\s+/, $stemp);
			if ($temp_name eq $entries[1])
			{
				$found = 1;
				last;
			}
...
foreach $type (@suffix)
{
	system("$prosys_dir/script/combine_fr_global.pl $prosys_dir/script/ . $qname.rank 0.5 $type 5 $type-comb.pir");
