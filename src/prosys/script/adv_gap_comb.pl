#!/usr/bin/perl -w
#################################################################################
#Combine two pir alignments according to minimum cover size using only covered
#segments + linkers
#Input: script dir, pir file 1, pir file 2, minimum size(15 or 20), linker length(10), output pir
#Depends on: analyze_pir_align.pl, combine_pir_align.pl
#Modifided from simple_gap_comb.pl
#Author: Jianlin Cheng
#Date: 9/18/2005.
#Obsolete: Replaced by combine_pir_align_adv.pl
#################################################################################
if (@ARGV != 5)
{
