...
#Input: script dir, option file, fasta dataset, file mapping file, output file.
...
#Date: 07/01/2005
#Modification: change from extract_feature_linda.pl to handle the file name mapping.
###############################################################################
if (@ARGV != 5)
{
	die "need five parameters: script dir, feature option file, fasta dataset(rych_full.fasta), filename mapping file,  output file.\n"; 
}
