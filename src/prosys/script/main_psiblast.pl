...
#if ( $exponent < -100 && $reso < 2.5 && $correct_ratio > 0.85)
{
	...
	`mv $fasta_file.easy$idx.pir psiblast$idx.pir`; 
	system("$script_dir/pir2ts_energy.pl $modeller_dir $atom_dir $work_dir psiblast$idx.pir $cm_model_num");

	`mv model.log $fasta_file.log`; 
	if (-f "$name.pdb")
	{
		`mv $name.pdb psiblast$idx.pdb`;
	}
