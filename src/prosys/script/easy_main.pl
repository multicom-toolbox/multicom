...
system("$script_dir/cm_parse_blast.pl $fasta_file.easy.blast $fasta_file.easy.local");
...
system("$script_dir/easy_align.pl $script_dir $fasta_file $fasta_file.easy.local $cm_min_cover_size $cm_max_gap_size $cm_max_linker_size $cm_evalue_comb $adv_comb_join_max_size $cm_evalue_diff $fasta_file.easy.pir");  
...
if ($add_stx_info_rm_identical eq "yes")
{
	...
	system("$script_dir/pir_proc_resolution.pl $fasta_file.easy.pir $chain_stx_info $rm_identical_resolution $fasta_file.pir.stx");
	if (-f "$fasta_file.pir.stx")
	{
		`mv $fasta_file.easy.pir $fasta_file.pir.nostx`;
		`mv $fasta_file.pir.stx $fasta_file.easy.pir`; 
	}

}

open(PIR, "$fasta_file.easy.pir") || die "can't generate pir file from local alignments.\n";
@pir = <PIR>;
close PIR; 
if (@pir <= 4)
{
...
print "evalue = $evalue, $correct_ratio ($ratio), resolution = $reso\n";
#if ( $exponent < -90 && $reso < 2.5 && $correct_ratio > 0.85)
#if ( $exponent < -100 && $reso < 2.5 && $correct_ratio > 0.85)
{
	print "Use Modeller to generate tertiary structures...\n"; 
	system("$script_dir/pir2ts_energy.pl $modeller_dir $atom_dir $work_dir $fasta_file.easy.pir $cm_model_num");

	`mv model.log $fasta_file.log`; 
	if (-f "$name.pdb")
	{
		`mv $name.pdb easiest.pdb`;
	}
...
#else
#{
#	print "The template doesn't meet conditions. No easy model is generated.\n";
#}
