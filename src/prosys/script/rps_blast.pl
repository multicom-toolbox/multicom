#!/usr/bin/perl -w
############################################################################
#use rpsblast to blast a sequence to a chk profile of target
...
if (@ARGV != 6)
{
	die "need six parameters:script dir,  blast dir, query file, target file, target chk file(naming: name.chk), output file.\n";
}
...
#secondary process
system("$blast_dir/copymat -P $uniq -r T");

#create database using formatdb
#system("$blast_dir/formatdb -i $uniq -p T -o T -l $uniq.log");
system("$blast_dir/formatdb -i $uniq -p T -l $uniq.log");

#searching the database using rpsblast
system("$blast_dir/rpsblast -i $query_fasta -d $uniq -o $out_file.rpsb -e 50 -l $uniq.blog");


#read and parse alignment results here : $out_file.blast
system("$script_dir/cm_parse_rpsblast.pl $out_file.rpsb $out_file.local");
...
if (@local < 3)
{
	...
	`rm $uniq*`; 
	`rm $out_file.rpsb $out_file.local`; 
	#clean-up (remove the tempoary files)
	#`rm $uniq*`; 
	die "no local alignments are generated from rps-blast, use default values.\n";
}
close LOCAL;
shift @local;
shift @local;
