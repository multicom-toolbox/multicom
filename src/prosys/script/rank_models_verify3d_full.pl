...
#rule 6: take models from palign,palign_ss, clustalw, tcoffee 
@methods = ("palign", "palign_ss", "clustalw", "tcoffee");

foreach $method (@methods)
{
	for ($i = 1; $i <= 10; $i++)
	{
		$candidate = "$output_dir/$method$i.pdb";	
		$pir = "$output_dir/$method$i.pir";
...
#reorder the model using v3d score
if ($bsuccess == 1)
{
...
for ($i = 0; $i < $size; $i++)
{
	...
	if (-f $source_file)
	{
		...
		print MODEL "vfold$idx.pdb\tcafasp$idx.pdb\t", $sorted[$i]{"model"}, "\t", $sorted[$i]{"type"},"\t", $sorted[$i]{"align"},"\t", $sorted[$i]{"score"}, "\n";
