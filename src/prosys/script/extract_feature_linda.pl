...
#Date: 05/16/2005
#Modification: change from extract_feature.pl to handle the file name mapping.
###############################################################################
if (@ARGV != 6)
{
	die "need six parameters: script dir, feature option file, fasta dataset(rych_full.fasta), pair list file(rank_oneway.txt), filename mapping file,  output file.\n"; 
}
