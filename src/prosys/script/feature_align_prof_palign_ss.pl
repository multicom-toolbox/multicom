#!/usr/bin/perl -w
#########################################################################
#Compute the palign profile alignment score (with ss info)
...
if (@ARGV != 8)
{
	die "need 8 parameters: palignp dir, query file(fasta), query pssm file,query ss file,  target file(fasta), target pssm file, target ss file, ouptut file.\n";
}
...
$query_ss = shift @ARGV;
-f $query_ss || die "can't find query ss file.\n";
...
$target_ss = shift @ARGV;
-f $target_ss || die "can't find target ss file.\n";

$out_file = shift @ARGV;

#do prof-prof alignment using palignp
#`$palign_dir/palignp -ali -seq $query_pssm - -prof $target_pssm - >$query_pssm.pal`;
`$palign_dir/palignp -ali -seq $query_pssm $query_ss - -prof $target_pssm $target_ss - >$out_file.pal`;
