...
{
	$modeller = "$modeller_dir/bin/mod9.16";
}
...
for ($i = 0; $i < $temp_num; $i++) 
{
	...
	#	print "start: $start...\n";
		...
		#		chdir $cur_dir; 
		#	$residue_name =  substr($atom_seq, $start-1,1); 
		#	warn "residue $start is $residue_name in pdb, but $char ($j) in pir alignment.\n"; 
			#disable the residue check because hhpred pdb does not contain all the residues sometime.
