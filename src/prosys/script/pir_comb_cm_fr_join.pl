...
#gap_stop_size, max_linker_size, join max size (optional, for adv comb only, e.g. 5), 
#output_file
#Notice: if max_linker_size < 0, using simple combination; otherwise,
#use advanced combination.
#Modified from pir_comb_cm_fr.pl
#add one new parameter(join max size) for advanced combination.
...
if (@ARGV != 7 && @ARGV != 8)
{
	die "need 7 or 8 parameters: script_dir, cm_pir_file, fr_pir_file_list, min_cover_size(20), gap_stop_size(20), max_linker_size(10), join max size(optional, e.g. 5, for adv comb only), output file\n";
}
...
{
	system("$script_dir/pir_adv_comb_join.pl $script_dir $cm_pir_file.can $min_cover_size $gap_stop_size $max_linker_size 1 $join_max_size $output_file");
}
