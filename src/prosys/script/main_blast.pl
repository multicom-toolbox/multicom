...
system("$script_dir/easy_align_more.pl $script_dir $fasta_file $fasta_file.easy.local $cm_min_cover_size $cm_max_gap_size $cm_max_linker_size $cm_evalue_comb $adv_comb_join_max_size $cm_evalue_diff $fasta_file.easy");  
...
#if ( $exponent < -100 && $reso < 2.5 && $correct_ratio > 0.85)
{
	...
	`mv $fasta_file.easy$idx.pir blast$idx.pir`; 
	system("$script_dir/pir2ts_energy.pl $modeller_dir $atom_dir $work_dir blast$idx.pir $cm_model_num");

	`mv model.log $fasta_file.log`; 
	if (-f "$name.pdb")
	{
		`mv $name.pdb blast$idx.pdb`;
	}
