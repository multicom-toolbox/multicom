...
#use other templates to fill large gap
$min_cover_size = 30; 
while (@neg_backup)
{
	$pir_temp = shift @neg_backup; 
	$pir_file = "$work_dir/$pir_temp.pir";
	if (-f $pir_file)
	{
		system("$script_dir/combine_pir_align_adv_join.pl $script_dir $output_file $pir_file $min_cover_size 1 -1 $output_file");
	}
}
