...
#Date: 4/12/2005
#Bug fix: 9/05/2005: evalue 0 is not ranked on the top.
###############################################################
...
$database = $blast[11];
chomp $database;
$database =~ /^Database/ || die "can't find database name\n";
$db_size = $blast[12];
chomp $db_size;
if ($db_size =~ /^\s+([\d,]+) sequences;/)
{
	$db_size = $1;
	$db_size =~ s/,//g; 
}
...
{
	die "can't find the size of database.\n"; 
}
print ALIGN "$query_name $query_length $database $db_size\n\n"; 
...
while (@blast)
{
	...
	if ($line =~ /^>(.+)/)
	{
		...
		else
		{
			...
			if ($score_exp =~ /Score =\s+([\d\.]+) bits.+Expect =\s+(\S+),\s+Method/)
			{
				$score = $1;
				$expe = $2; 
			}
