...
system("$sam_dir/bin/buildmodel $name -train $fasta_file.fas -randseed 0"); 
...
system("$sam_dir/bin/hmmscore $name -calibrate 1  -i $name.mod -db $samdb -sw 2");
...
system("$sam_dir/bin/align2model $name -i $name.mod -db $name.sel");
#############################################################################
