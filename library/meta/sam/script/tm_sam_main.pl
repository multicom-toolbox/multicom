...
system("$sam_dir/bin/w0.5 $fasta_file.fas $name.mod"); 

#search sam hhm against the database
print "search SAM HMM against database ...\n";
#print("$sam_dir/bin/hmmscore $name -calibrate 1  -i $name.mod -db $samdb -sw 2\n");
system("$sam_dir/bin/hmmscore $name -calibrate 1  -i $name.mod -db $samdb -sw 2 -dpstyle 0 -adpstyle 5");
...
system("$meta_dir/script/rank_templates.pl $filename.dist $work_dir/$name.rank");
#####################################################################################
...
system("$sam_dir/bin/align2model $name -i $name.mod -db $name.sel -adpstyle 5");
#############################################################################
...
while (@align)
{
...

	{
		...
		#here, there is a bug, we should report template sequence length instead of query sequence. to fix: 1/8/2009 
...
system("$meta_dir/script/main_sam_easy.pl $option_file $fasta_file $work_dir");

print "Comparative modelling for $qname is done.\n"; 
