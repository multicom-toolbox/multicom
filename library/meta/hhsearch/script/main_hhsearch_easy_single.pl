...
while ($model_idx <= $easy_model_num && @easy_cm >= 5)
{
	...
	{
		system("$script_dir/cm_align_blast.pl $fasta_file $fasta_file.easy $cm_align_evalue $cm_max_gap_size $cm_min_cover_size hh$model_idx.pir");
	}
	else #use a new version of combination
	{
		...
		system("$meta_dir/script/hhsearch_align_comb.pl $script_dir $fasta_file $fasta_file.easy $cm_min_cover_size $cm_max_gap_size $cm_max_linker_size $cm_evalue_comb $adv_comb_join_max_size $cm_evalue_diff hh$model_idx.pir");  
	}
