...
#	$join_max_size = -1; 
	...
	my $sim_threshold = 0.72;  #changed on May 20, 2014. human prediction: 0.70 was used. 
...
while ($model_idx <= $easy_model_num && @easy_cm >= 5)
{
	...
	system("$script_dir/pir2ts_energy.pl $modeller_dir $atom_dir $work_dir $output_prefix_name$model_idx.pir $cm_model_num");
	`rm model.log`; 
