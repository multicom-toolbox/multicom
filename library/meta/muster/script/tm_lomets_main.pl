...
print "Use Lomets to do fold recognition...\n";
`cp $fasta_file $work_dir/seq.fasta`; 
system("$itasser_dir/I-TASSERmod/runOneLOMETS.pl -libdir $itasser_dir -seqname $name -datadir $work_dir -usrname chengji"); 

#generate ranking list, and parse the output into local alignment format
open(INIT, "initres.LOMETS") || die "can't read initres.LOMETS\n";
@init = <INIT>;
close INIT;
shift @init; 
`mv initres.LOMETS initres.GGGd`; 

open(RANK, ">$name.lomets.rank") ||die "can't create $name.lomets.rank.\n";
...
for ($i = 1; $i <= 10; $i++)
{
	$org_align = "align_GGGd_$i.txt";
	$new_pir = "lomets$i.pir";
	$atom_file = "threading_GGGd_$i.pdb";
	if (! -f $org_align)
	{
		next;
	}
	...
	system("$script_dir/clash_check_broken.pl $fasta_file lomets$i.pdb > clash$i.txt"); 
	...
	foreach $entry (@clash)
	{
		...
		{
			print "lomets$i.pdb's chain is broken, removed!\n";
			`mv lomets$i.pdb lomets$i.pdb.broken`; 
...
system("$meta_dir/script/filter_rank.pl $pdb_db_dir/pdb_cm $name.lomets.rank $name.lomets.filter.rank"); 

print "One LOMETS (GGGd) comparative modeling is done.\n";
