...
if (@ARGV != 5)
{
	die "need 5 parameters: pir alignment 1, pir alignment 2, left expansion limit, right expansion limit, and output alignment file.\n";
}
...
#get unchanged regions in the first alignment
$length = length($t_align1);

$q_start1 = 1;  #q_start1 is used to record the range cover by the template
for ($i = 0; $i < $length && substr($t_align1, $i, 1) eq "-"; $i++)
{
	if (substr($q_algn1, $i, 1) ne "-")
	{
		$q_start1++; 
	}
}
...
$q_end1 = $length; 
for ($i = $length - 1; $i >= 0 && substr($t_align1, $i, 1) eq "-"; $i--)
{
	if (substr($q_align1, $i, 1) ne "-")
	{
		$q_end1--; 
	}
}
...
#get the cores (the region that will be kept)
$t_core1 = substr($t_align1, $core_left_index, $core_right_index - $core_left_index + 1); 
$q_core1 = substr($q_align1, $core_left_index, $core_right_index - $core_left_index + 1); 
...
#get the boundaries of the left and right fragments from the alignment 2
$length = length($t_align2); 
$aa_ord = $t_start2 - 1; 
...
for ($i = 0; $i < $length; $i++)
{
	...
	if (substr($t_align2, $i, 0) ne "-")
	{
		$aa_ord++; 	
		if ($aa_ord >= $left_limit && $aa_ord < $t_start1 && $left_aa_start == -1)
		{
			$left_aa_start = $aa_ord; 	
			$left_idx_start = $i; 
		}

		if ( ($aa_ord == $t_start1 - 1 || $aa_ord == $t_end2) && $left_aa_end == -1)
		{
			$left_aa_end = $aa_ord; 
			$left_idx_end = $i; 	
		}
		...
		if ( $aa_ord == $t_end1 + 1)
		{
			$right_aa_start = $aa_ord;
			$right_idx_start = $i; 
		}
		
		if ( ($aa_ord == $t_end2 || $ $aa_ord == $right_limit) && $right_aa_end == -1)
		{
			$right_aa_end = $aa_ord; 
			$right_idx_end = $i; 
		}
