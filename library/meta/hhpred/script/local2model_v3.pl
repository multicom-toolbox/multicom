...
while ($model_idx <= $easy_model_num && @easy_cm >= 5)
{
	...
	#check if this file is the same as previous file, if so, don't generate models and remove the file

	$redundant_pir_file = 0; 
	open(REDUNDANT, "$output_prefix_name$model_idx.pir");
	@current_pir = <REDUNDANT>;
	close REDUNDANT;
	$current_stx = $current_pir[2]; 

	for ($jj = 1; $jj < $model_idx; $jj++)
	{
		open(HIGH, "$output_prefix_name$jj.pir");
		@high_pir = <HIGH>;
		close HIGH;
		$high_stx = $high_pir[2]; 

		if ($current_stx eq $high_stx)
		{
			print "$output_prefix_name$model_idx.pir is redundant to $output_prefix_name$jj.pir, skip it.\n";
			$redundant_pir_file = 1; 
			last;
		}	
	}

	#system("$script_dir/pir2ts_energy_pdb.pl $modeller_dir $atom_dir $work_dir $output_prefix_name$model_idx.pir $cm_model_num");
	if ($redundant_pir_file == 0)
	{
		system("$script_dir/pir2ts_energy_pdb.pl $modeller_dir $temp_dir $work_dir $output_prefix_name$model_idx.pir $cm_model_num");
	}
	else
	{
		`rm $output_prefix_name$model_idx.pir`;  
	}
	`rm model.log`; 
