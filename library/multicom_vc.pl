...
while (<OPTION>)
{
	$line = $_; 
	chomp $line;
	if ($line =~ /^multicom_dir/) #/storage/htc/bdm/tools/MULTICOM_CLUSTER/sunflower/chengji/casp8/meta/
	{
	...
	if ($line =~ /^meta_option_full_length/) #/storage/htc/bdm/tools/MULTICOM_CLUSTER/sunflower/chengji/casp8/meta/test_system/multicom_option_casp12
	{
		($other, $value) = split(/=/, $line);
		$value =~ s/\s//g; 
	...
	if ($line =~ /^meta_option_easy_domain/) #/storage/htc/bdm/tools/MULTICOM_CLUSTER/sunflower/chengji/casp8/meta/test_system/multicom_option_casp12
	{
		($other, $value) = split(/=/, $line);
		$value =~ s/\s//g; 
	...
	if ($line =~ /^meta_option_hard_domain/) #/storage/htc/bdm/tools/MULTICOM_CLUSTER/sunflower/chengji/casp8/meta/test_system/multicom_option_hard_casp12
	{
		($other, $value) = split(/=/, $line);
		$value =~ s/\s//g; 
	...
	if ($line =~ /^prosys_dir/) #/storage/htc/bdm/tools/MULTICOM_CLUSTER/sunflower/chengji/software/prosys/
	{
		($other, $value) = split(/=/, $line);
		$value =~ s/\s//g; 
		$prosys_dir = $value; 
	...
	if ($line =~ /^modeller_dir/) #/storage/htc/bdm/tools/modeller/modeller-9.16/
	{
		($other, $value) = split(/=/, $line);
		$value =~ s/\s//g; 
		$modeller_dir = $value; 
...
{
	system("$multicom_dir/meta/script/multicom_server_vc.pl $meta_option_full_length $query_file $full_length_dir"); 
  #/storage/htc/bdm/tools/MULTICOM_CLUSTER/MULTICOM_lite/library/
  #/storage/htc/bdm/tools/MULTICOM_CLUSTER/MULTICOM_lite/library/meta/test_system/multicom_option_casp12
}
#########################################################################################################
...
elsif (-f $hhsearch_local_alignment)
{
   ...
   #/storage/htc/bdm/tools/MULTICOM_CLUSTER/sunflower/chengji/casp8/meta/
   # /storage/htc/bdm/tools/MULTICOM_CLUSTER/sunflower/chengji/casp8/meta/test_system/multicom_option_hard_casp12
...
if (@domain_start > 1) #multiple domain case
{
	...
	for ($i = 0; $i < @domain_start; $i++)
	{
        ...
        #/storage/htc/bdm/tools/MULTICOM_CLUSTER/sunflower/chengji/casp8/meta/
        #/storage/htc/bdm/tools/MULTICOM_CLUSTER/sunflower/chengji/casp8/meta/test_system/multicom_option_casp12
			}
			#######################################################################################
		}
		else
...
if ($domain_split_comb == 1)
{
   ...
   #/storage/htc/bdm/tools/MULTICOM_CLUSTER/sunflower/chengji/software/prosys/
   #/storage/htc/bdm/tools/modeller/modeller-9.16/
